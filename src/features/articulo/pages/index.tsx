...
import { Search, Plus, Edit, Trash2, Package, ChevronLeft, ChevronRight, BadgeDollarSign } from "lucide-react";
        ...
        <div className={stack({ gap: '6', p: '6' })}>
            ...
            <div className={hstack({ justifyContent: 'space-between' })}>
                <div className={hstack({ gap: '3' })}>
                    ...
                    <div>
                        <h1 className={css({ fontSize: '2xl', fontWeight: '800', color: '#1A365D' })}>Artículos</h1>
                        <p className={css({ fontSize: 'sm', color: 'gray.500' })}>Gestioná tu inventario, precios y stock</p>
                    </div>
                </div>
            ...
            <div className={cardStyle}>
                <table className={css({ w: 'full', borderCollapse: 'collapse' })}>
                    ...
                    <tbody>
                        {isLoading ? (
                            <tr><td colSpan={6} className={css(tdStyle)}>Cargando artículos...</td></tr>
                        ) : data?.items?.map((articulo: any) => (
                            <tr key={articulo.id} className={css({
                                ...
                                <td className={css(tdStyle)}>
                                    <code className={css({ fontSize: 'xs', bgColor: 'blue.50', color: 'blue.700', px: '2', py: '1', borderRadius: 'md', fontWeight: 'bold' })}>
                                        {articulo.codigo}
                                    </code>
                                </td>
                                <td className={css(tdStyle)}>
                                    <div className={stack({ gap: '0' })}>
                                        <span className={css({ fontWeight: '700', color: 'gray.800' })}>{articulo.nombre}</span>
                                        <span className={css({ fontSize: 'xs', color: 'gray.400' })}>{articulo.descripcion || 'Sin descripción'}</span>
                                    </div>
                                </td>
                                <td className={css(tdStyle)}>
                                ...
                                <td className={css(tdStyle)}>
                                    <span className={css({
                                        fontWeight: 'bold',
                                        color: articulo.stockActual <= 0 ? 'red.500' : 'gray.700'
                                    })}>
                                        {articulo.stockActual} <small className={css({ fontWeight: 'normal', color: 'gray.400' })}>{articulo.unidadMedida}</small>
                                    </span>
                                </td>
                                <td className={css(tdStyle)}>
                                    <span className={css({
                                        ...
                                        backgroundColor: articulo.activo ? '#28a745' : '#dc3545'
                                    })}>
                                ...
                                <td className={`${tdStyle} ${css({ textAlign: 'right' })}`}>
                                    <div className={hstack({ gap: '1', justifyContent: 'flex-end' })}>
                                        ...
                                        <ConfirmDialog
                                            ...
                                            trigger={
                                                <button className={css({ color: 'red.500', cursor: 'pointer', p: '2' })}>
                                                    <Trash2 size={18} />
                                                </button>
                                            }
                                        />
            ...
            <div className={hstack({ justifyContent: 'space-between', px: '2' })}>
                <p className={css({ fontSize: 'sm', color: 'gray.500', fontWeight: '500' })}>
                    Mostrando <span className={css({ color: 'blue.600', fontWeight: 'bold' })}>{data?.items?.length || 0}</span> de {data?.totalCount || 0} artículos
                </p>
