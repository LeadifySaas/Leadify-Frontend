...
import type { ArticuloInterno, ArticuloInternoFormValues } from "../schemas";

export const useArticulosInterno = (page?: number, size?: number, search?: string) => {
    ...
    const createMutation = useMutation({
        mutationFn: (nuevo: ArticuloInternoFormValues) => articuloApi.create(nuevo),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["articulos"] }),
    });

    const updateMutation = useMutation({
        mutationFn: (data: { id: number } & ArticuloInternoFormValues) => articuloApi.update(data.id, data),
