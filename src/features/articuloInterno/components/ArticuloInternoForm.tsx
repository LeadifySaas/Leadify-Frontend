...
import { articuloInternoSchema, type ArticuloInternoFormValues } from "../schemas";
import { useArticulosInterno } from "../hooks/useArticulosInterno";
...
export function ArticuloInternoForm({ mode, initialData }: Props) {
    const navigate = useNavigate();
    const { createArticulo, updateArticulo, isCreating, isUpdating } = useArticulosInterno();
    ...
    const {
        ...
        reset,
        formState: { errors }
    } = useForm<ArticuloInternoFormValues>({
        resolver: zodResolver(articuloInternoSchema),
    ...
    const onSubmit = async (data: ArticuloInternoFormValues) => {
        try {
            ...
            navigate({ to: '/materiales/articulosInternos'});
        } catch (error: any) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className={stack({ gap: '6', p: '2' })}>
            ...
            <div className={hstack({ justifyContent: 'space-between' })}>
                <div className={hstack({ gap: '4' })}>
                    <button
                        type="button"
                        onClick={() => navigate({ to: '/materiales/articulosInternos' })}
                    ...
                    <h1 className={css({ fontSize: '2xl', fontWeight: '800', color: '#1A365D' })}>
                        {mode === 'create' ? 'Nuevo Artículo Interno' : 'Editar Artículo Interno'}
                    </h1>
                </div>
                <button type="submit" disabled={isCreating || isUpdating} className={saveBtnStyle}>
                    <Save size={18} /> {isCreating || isUpdating ? 'Guardando...' : 'Guardar Artículo Interno'}
                </button>
            </div>
                    ...
                    <div className={cardStyle}>
                        ...
                        <div className={grid({ columns: 4, gap: '4' })}>
                            ...
                            <div className={stack({ gap: '1.5' })}>
                                ...
                                <input type="number" step="0.01" {...register("precioCompra", { valueAsNumber: true })} className={inputStyle} />
                                {errors.precioCompra && <span className={errorStyle}>{errors.precioCompra.message}</span>}
                            </div>
                            <div className={stack({ gap: '1.5' })}>
