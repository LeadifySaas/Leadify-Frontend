...
export function RemitoForm({
  ...
  const sedesFiltradas = sedes.filter(
    (s: any) => Number(s.clienteId) === Number(selectedClienteId)
  );
    ...
    try {
      ...
      setErrorStatus({
        ...
        detail:
          ...
          'Error desconocido al procesar el remito.'
      });
      toast.error('Ocurrió un error al guardar el remito.');
    }
  };

  const getFieldStyle = (hasError: any) =>
    css(inputStyleRaw, {
      ...
      >
        ...
        <div className={hstack({ justifyContent: 'space-between', mb: '4' })}>
          <div className={hstack({ gap: '4' })}>
            ...
            >
              {mode === 'create'
                ? 'Nuevo Remito'
                : `Editar Remito #${initialData?.numeroRemito}`}
            </h1>
          </div>
          <button
            type="submit"
          ...
          >
            <Save size={18} />{' '}
            {isCreating || isUpdating ? 'Guardando...' : 'Guardar Remito'}
          </button>
        </div>
        ...
        <div className={grid({ columns: { base: 1, lg: 3 }, gap: '6' })}>
          ...
          <div className={css({ lg: { gridColumn: 'span 2' }, spaceY: '6' })}>
              ...
              <div className={grid({ columns: 2, gap: '4' })}>
                <div className={stack({ gap: '1.5', gridColumn: 'span 2' })}>
                  ...
                  <select
                    ...
                    className={getFieldStyle(errors.clienteId)}
                  >
                ...
                <div className={stack({ gap: '1.5', gridColumn: 'span 2' })}>
                  ...
                  <select
                    ...
                    className={getFieldStyle(errors.sedeId)}
                  ...
                  {errors.sedeId && (
                    <span className={errorStyle}>{errors.sedeId.message}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className={cardStyle}>
              <div
                className={hstack({ justifyContent: 'space-between', mb: '4' })}
              >
                ...
                <button
                  type="button"
                  onClick={() =>
                    append({ articuloId: 0, cantidad: 1, notas: '' })
                  }
                  className={addItemBtnStyle}
                >
                ...
                {fields.map((field, index) => (
                  <div key={field.id} className={itemRowStyle}>
                    ...
                    <div className={grid({ columns: 4, gap: '3' })}>
                      <div
                        className={stack({ gap: '1', gridColumn: 'span 3' })}
                      >
                        <select
                          {...register(`items.${index}.articuloId` as const, {
                            valueAsNumber: true
                          })}
                          className={getFieldStyle(
                            errors.items?.[index]?.articuloId
                          )}
                        >
                      ...
                      <div
                        className={stack({ gap: '1', gridColumn: 'span 4' })}
                      >
                        <input
                          placeholder="Notas del artículo (opcional)"
                          {...register(`items.${index}.notas` as const)}
                          className={notesInputStyle}
                        />
                      </div>
                    </div>
                  </div>
              ...
              {errors.items && (
                <span className={errorStyle}>{errors.items.message}</span>
              )}
            </div>
          </div>
          ...
          <div className={stack({ gap: '6' })}>
            <div className={cardStyle}>
              ...
              <div className={stack({ gap: '4' })}>
                <div className={stack({ gap: '1.5' })}>
                  ...
                  <input
                    placeholder="Ej: R0001-00123"
                    {...register('numeroRemito')}
                    className={getFieldStyle(errors.numeroRemito)}
                  />
                  {errors.numeroRemito && (
                    <span className={errorStyle}>
                      {errors.numeroRemito.message}
                    </span>
                  )}
                </div>
                <div className={stack({ gap: '1.5' })}>
                  ...
                  <input
                    ...
                    className={getFieldStyle(errors.fechaEmision)}
                  />
                  {errors.fechaEmision && (
                    <span className={errorStyle}>
            ...
            <div className={cardStyle}>
              ...
              <div className={stack({ gap: '1.5' })}>
                <textarea
                  rows={4}
                  placeholder="Notas generales del remito..."
                  {...register('observaciones')}
                  className={getFieldStyle(errors.observaciones)}
                ></textarea>
              </div>
            </div>
          </div>
...
const inputStyleRaw = {
  ...
  _disabled: { bgColor: 'gray.100', color: 'gray.400', cursor: 'not-allowed' }
};
...
const addItemBtnStyle = hstack({
  ...
  fontWeight: 'bold',
  cursor: 'pointer',
  _hover: { bgColor: 'blue.50', borderColor: 'blue.200' }
});
...
const deleteRowBtnStyle = center({
  color: 'gray.400',
...
const notesInputStyle = css({
  ...
  px: '1',
  fontSize: 'xs',
  _focus: { borderColor: 'blue.400', boxShadow: 'none', bgColor: 'transparent' }
});
