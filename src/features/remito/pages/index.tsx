...
import { Search, Plus, Edit, Trash2, FileText, ChevronLeft, ChevronRight, FileDown, RefreshCw, FileSpreadsheet } from "lucide-react";
...
export default function RemitosPage() {
    ...
    const { remitos, totalCount, isLoading, deleteRemito, updateStatus } = useRemitos(page, size, search);

    const [statusModal, setStatusModal] = useState<{ isOpen: boolean; remito: any }>({
        isOpen: false,
        remito: null
    });
    ...
    const handleDownloadPdf = async (id: number) => {
        try {
            ...
            alert("No se pudo generar el PDF. Verifica que el remito tenga todos los datos cargados.");
        }
    };
        ...
        <div className={stack({ gap: '6', p: '6' })}>
            ...
            <div className={hstack({ justifyContent: 'space-between' })}>
                <div className={hstack({ gap: '3' })}>
                    ...
                    <div>
                        <h1 className={css({ fontSize: '2xl', fontWeight: '800', color: '#1A365D' })}>Remitos</h1>
                        <p className={css({ fontSize: 'sm', color: 'gray.500' })}>Gestioná los remitos y comprobantes</p>
                    </div>
                </div>
            ...
            <div className={cardStyle}>
                <table className={css({ w: 'full', borderCollapse: 'collapse' })}>
                    ...
                    <tbody>
                        {isLoading ? (
                            <tr><td colSpan={5} className={css(tdStyle, { textAlign: 'center', py: '10' })}>Cargando remitos...</td></tr>
                        ) : (!remitos || remitos.length === 0) ? (
                            <tr>
                                <td colSpan={5} className={css(tdStyle, { textAlign: 'center', py: '10' })}>
                            ...
                            <tr key={r.id} className={rowStyle}>
                                ...
                                <td className={css(tdStyle)}>
                                    <span className={css({ fontWeight: '500', color: 'gray.700' })}>
                                        {r.fechaEmision ? new Date(r.fechaEmision).toLocaleDateString() : '---'}
                                    </span>
                                </td>
                                <td className={css(tdStyle)}>
                                    <span className={css({ fontWeight: '500', color: 'gray.700' })}>
                                ...
                                <td className={`${css(tdStyle)} ${css({ textAlign: 'right' })}`}>
                                    <div className={hstack({ gap: '1', justifyContent: 'flex-end' })}>
                                        ...
                                        <button
                                            ...
                                            onClick={() => setStatusModal({ isOpen: true, remito: r })}
                                        >
                                            <RefreshCw size={16} />
                                        </button>
                                        ...
                                        <ConfirmDialog
                                            ...
                                            trigger={
                                                <button className={css({ color: 'red.400', cursor: 'pointer', p: '2', borderRadius: 'lg', _hover: { color: 'red.600', bgColor: 'red.50' } })}>
                                                    <Trash2 size={18} />
                                                </button>
                                            }
                                        />
            ...
            <div className={hstack({ justifyContent: 'space-between', px: '2' })}>
                <p className={css({ fontSize: 'sm', color: 'gray.500', fontWeight: '500' })}>
                    Mostrando <span className={css({ color: 'blue.600', fontWeight: 'bold' })}>{remitos?.length || 0}</span> de {totalCount || 0} remitos
                </p>
...
const thStyle = { px: '6', py: '4', fontSize: 'sm', fontWeight: '800', color: 'gray.600', textTransform: 'uppercase', letterSpacing: 'wider' };
const tdStyle = { px: '6', py: '4', fontSize: 'md', color: 'gray.700' };
const rowStyle = css({ borderBottom: '1px solid', borderColor: 'gray.50', _hover: { bgColor: 'gray.50/50' }, transition: 'colors 0.2s' });

const btnPrimaryStyle = hstack({ px: '5', py: '2.5', bgColor: 'blue.600', color: 'white', borderRadius: 'xl', fontWeight: 'bold', cursor: 'pointer', _hover: { bgColor: 'blue.700' }, transition: 'all 0.2s' });

const btnExcelStyle = hstack({
    px: '5', py: '2.5', bgColor: 'green.600', color: 'white', borderRadius: 'xl',
    fontWeight: 'bold', cursor: 'pointer', _hover: { bgColor: 'green.700' }, transition: 'all 0.2s'
});

const inputStyle = css({ w: 'full', p: '2.5', pl: '10', bgColor: 'white', border: '1px solid', borderColor: 'gray.200', borderRadius: 'xl', fontSize: 'sm', outline: 'none', _focus: { borderColor: 'blue.400' } });
...
const numeroRemitoStyle = css({ fontSize: 'sm', bgColor: 'blue.50', color: 'blue.700', px: '2', py: '1', borderRadius: 'md', fontWeight: 'bold' });

const statusBadgeStyle = (status: string) => {
    const baseStyle = { px: '3', py: '1', borderRadius: 'md', fontSize: 'sm', fontWeight: 'bold', shadow: 'sm' } as const;
